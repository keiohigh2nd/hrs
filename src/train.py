...
target = T[1].reshape(1, len(T[1]))
align_target = np.zeros(len(target[0]))

for k in xrange(len(target[0])):
  align_target[k] = target[0][k]


N = 400
x_train, x_test = np.split(T[0].astype(np.float32),   [N])
y_train, y_test = np.split(align_target.astype(np.int32), [N])
N_test = y_test.size
